...
For example, to recreate the conda environment in which the package was built, you can run the following:
```
cd /opt/conda/envs/biobuild/conda-bld/linux-64
conda create -y -n debug -c ./ trtools
conda activate debug
```
If the build succeeded, you can also rerun the failed `bioconda-utils build` command with the `--keep-old-work` flag.
